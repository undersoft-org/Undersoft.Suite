            ...
            using (IServiceScope scope = setup.Manager.CreateScope())
            {
                try
                {
                    ...
                    )
                    {
                        ...
                        {
                            providerEntries.ForEach(provider => provider.Value.BaseCurrencyId = provider.Value.BaseCurrency!.Id).Commit();
                            var currencies = providerEntries.Select(provider => provider.Value.BaseCurrency).GroupBy(c => c.CurrencyCode).ForEach(h => h.FirstOrDefault()).Commit();
                            ...
                            servicer
                                        .Send(
                                            ...
                                            >(
                                                ...
                                                currencies.ForEach(c => c.PatchTo<Contracts.Currency>()).Commit())
                                            ).Wait();
                            ...
                            servicer
                                       .Send(
                                           ...
                                           >(
                                               EventPublishMode.PropagateCommand,
                                               providerEntries.ForEach(c => c.Value.PatchTo<Contracts.CurrencyProvider>()).Commit())
                                           ).Wait();
