...
[OpenData]
public abstract class OpenCqrsDetalizeController<TKey, TEntry, TReport, TEntity, TDto, TService>
...
{
    protected OpenCqrsDetalizeController() { }

    public OpenCqrsDetalizeController(IServicer servicer) : base(servicer) { }
    ...
    {
        return _servicer.Report(new DetalizedGet<TReport, TEntity, TDto>()).Result.Result;
    }
    ...
    public override SingleResult<TDto> Get([FromRoute] TKey key)
    {
        return new SingleResult<TDto>(
           _servicer
               .Report(
                   new DetalizedFind<TReport, TEntity, TDto>(
