namespace Undersoft.SDK.Cryptography
{
    ...
    internal class CryptoCode
    {
        internal const int ROUNDS = 16;

        internal RandomNumberGenerator randomSource = RandomNumberGenerator.Create();
        ...
        public CryptoCode(string cipherKey)
        {
            IV = new byte[8];
            randomSource.GetBytes(IV, 0, 8);
            SetupKey(cipherKey.GetBytes());
        }

        public CryptoCode(string cipherVector, string cipherKey)
        {
            IV = cipherVector.UniqueBytes64();
            SetupKey(cipherKey.GetBytes());
        }

        public CryptoCode(byte[] cipherVector, byte[] cipherKey)
        {
            IV = cipherVector;
            SetupKey(cipherKey);
        }

        public CryptoCode(byte[] cipherKey)
        {
            IV = new byte[8];
            randomSource.GetBytes(IV, 0, 8);
            SetupKey(cipherKey);
